...
def cerrarVentana():
    ...
    w_sc = exit_popup.winfo_screenwidth()
    h_sc = exit_popup.winfo_screenheight()
    x = (w_sc/2) - (width/2)
    y = (h_sc/2) - (height/2)
    #exit_popup.geometry("250x120")
    exit_popup.geometry('%dx%d+%d+%d' % (width, height, x, y)) ## Para spawnear la ventana en el medio de la pantalla
