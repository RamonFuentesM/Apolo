...
def setData(n_scanner, fils, table, ventana_historial): # Esta funcion es para traer los datos de la tabla almacenados en el archivo
    ...
    num_abrir_button = 0.218 # Variable para controlar la posicion de los botones en el historial
    filas = fils 


# def checkShedule(t, createScanButton, raiz):
#     # Comprobamos cada 1 segundo si el hilo ha terminado
#     raiz.after(1000, checkThread(t, createScanButton, raiz), t)

# def checkThread(t, createScanButton, raiz):
#     if not t.is_alive():  # Comprobamos si el hilo ha terminado para reactivar el boton
#         createScanButton.configure(state=NORMAL)
#     else:  # Si no ha terminado seguimos comprobando si el hilo termina
#         checkShedule(t, createScanButton, raiz)

def comprobarCampos(output_text, output, ventanaParametros, createScanButton, cancelButton, botones):
    
        global abortar, comandos, flag_norellenado, hosts, scanners
        hosts = []
        scanners = []
        comandos = []
...
def cryptoCommands():
    ...
    if objetivo_entry_cryto.get() != "" and (escaneres_crypto[0] or escaneres_crypto[1]):
        if escaneres_crypto[0]:
            comandos.append("Herramientas/testssl.sh/testssl.sh --vulnerable --protocols " + objetivo_entry_cryto.get() + " | tee reportes/Escaner" + str(num_scanner+1) + "/TLS_SSL_CryptoFailures.txt")
...
def checkTool(tools_activadas, ventanaParametros):
    tool_parameters = {}
    #tool_parameters = {"brokenAccess":brokenAccess,"cryptoFailures":cryptoFailures,"idAutFailures":idAutFailures,"vulcomp":vulnComp,"ssrf":ssrf,"infoGathering":infoGathering}
...
def catDirectiorio(directorio):
    ...
    if hay_txt:
        ...
        os.system(var)
    if hay_html:
        os.system("firefox "+ directorio + "*.html")
