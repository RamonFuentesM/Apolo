...
if [[ $EUID -ne 0 ]]; then    
	echo "Necesitas sudo para realizar la instalacion"     
	exit 1 
fi
...
error "Ocurrio un error al instalar WhatWeb"
...
cd ..
error "Ocurrio un error al instalar theHarvester"
...
echo "Descargando Dependency-check..."
sleep 1
unzip dependency-check-7.1.1-release.zip
set JAVA_HOME=%PROGRAMFILES%\Java\jdk-13.0.2
sudo apt install maven -y
error "Ocurrio un error al instalar Dependency-check"

# Instalar Bolt CSRF
echo "Descargando Bolt CSRF..."
sleep 1
sudo pip install -r Bolt/requirements.txt
error "Ocurrio un error al instalar Bolt CSRF"
